...
Uses Resend to send:
  1. Lead notification to scott@sgbpainting.com
...
app = Flask(__name__)
CORS(app)  # Allow cross-origin requests from Snapps iframe
...
ZAPIER_WEBHOOK   = os.environ.get("ZAPIER_WEBHOOK_URL", "")  # Optional: add Zapier webhook URL when ready
...
def get_sheets_service():
    """Build Google Sheets service from env var JSON or local file."""
...
# ===== LEAD NOTIFICATION EMAIL (to Scott) =====
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4f8;padding:30px 0;">
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(5,45,92,0.10);">
      ...
      <!-- Header -->
      <tr>
        <td style="background:#052D5C;padding:24px 28px;">
      ...
      <!-- Alert Banner -->
      <tr>
        <td style="background:#D91D23;padding:12px 28px;">
      ...
      <tr>
        <td style="padding:24px 28px 0;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#5a6a7e;width:130px;">Name</td>
              <td style="padding:6px 0;font-size:14px;font-weight:600;color:#1a1a2e;">{data.get('name', 'N/A')}</td>
            </tr>
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#5a6a7e;">Phone</td>
              <td style="padding:6px 0;font-size:14px;font-weight:600;color:#1a1a2e;">{data.get('phone', 'N/A')}</td>
            </tr>
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#5a6a7e;">Email</td>
              <td style="padding:6px 0;font-size:14px;font-weight:600;color:#1a1a2e;">{data.get('email', 'N/A')}</td>
            </tr>
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#5a6a7e;">Address</td>
              <td style="padding:6px 0;font-size:14px;color:#1a1a2e;">{data.get('address', 'Not provided')}</td>
            </tr>
          </table>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:20px 28px 0;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#5a6a7e;width:130px;">Project Type</td>
              <td style="padding:6px 0;font-size:14px;font-weight:600;color:#1a1a2e;">{fmt_project_type(data.get('projectType', ''))}</td>
            </tr>
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#5a6a7e;">Paint Tier</td>
              <td style="padding:6px 0;font-size:14px;color:#1a1a2e;">{fmt_tier(data.get('paintTier', ''))}</td>
            </tr>
      ...
      <tr>
        <td style="padding:20px 28px 24px;">
          <p style="font-size:12px;color:#999;margin:0;text-align:center;">This lead was generated via the SGB Custom Painting Cost Calculator. Calculator built by <a href="https://www.petersonseoconsulting.com" style="color:#2D71AE;">Peterson SEO</a>.</p>
        </td>
      </tr>
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4f8;padding:30px 0;">
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(5,45,92,0.10);">
      ...
      <!-- Header -->
      <tr>
        <td style="background:#052D5C;padding:28px 28px 22px;text-align:center;">
      ...
      <tr>
        <td style="padding:20px 28px 0;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#333;">&#10003; &nbsp;25+ Years of Experience in Chico and Northern California</td>
            </tr>
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#333;">&#10003; &nbsp;Licensed, Insured and EPA Lead Safe Certified</td>
            </tr>
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#333;">&#10003; &nbsp;PDCA Accredited - Professional Painting Standards</td>
            </tr>
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#333;">&#10003; &nbsp;No-Surprise Estimates and Transparent Pricing</td>
            </tr>
            <tr>
              <td style="padding:6px 0;font-size:14px;color:#333;">&#10003; &nbsp;Premium Paints from Sherwin-Williams, Benjamin Moore and Dunn-Edwards</td>
            </tr>
          </table>
        </td>
      </tr>
      ...
      <!-- Footer -->
      <tr>
        <td style="padding:22px 28px 24px;border-top:1px solid #e8edf2;margin-top:20px;">
...
# ===== ROUTE =====
@app.route("/send-estimate", methods=["POST", "OPTIONS"])
def send_estimate():
    ...
    if ZAPIER_WEBHOOK:
        try:
            zapier_payload = {
                ...
                "cab_doors":     data.get("cabDoors", 0),
                "cab_drawers":   data.get("cabDrawers", 0),
...
def health():
    return jsonify({"status": "ok", "service": "SGB Painting Calculator Email API"}), 200
